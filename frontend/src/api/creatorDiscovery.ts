...
export const creatorDiscoveryApi = {
  ...
  importRun: async (file: File, payload: Omit<DiscoveryRequest, "inputs">) => {
    ...
    form.append("mode", payload.mode); form.append("auto_process", String(payload.auto_process));
  ...
  candidates: (offset = 0, status = "OPEN") => api<Page<Candidate>>(`/creator-discovery/candidates?offset=${offset}&limit=50&review_status=${encodeURIComponent(status)}`),
  ...
  export: (format: "csv" | "xlsx", extended: boolean, runUid?: string, template: "standard" | "intelligence" = "standard") => download(
    "/creators/export",
    { format, extended, run_uid: runUid || null, template },
    format === "xlsx" ? (template === "intelligence" ? "creator_intelligence_report.xlsx" : "creator_discovery_export.xlsx") : "creator_discovery.csv",
  ),
};
